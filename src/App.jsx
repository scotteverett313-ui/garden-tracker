...
// ─── What to plant next logic ─────────────────────────────────────────────────
function getNextPlantSuggestions(zone, plants, frostDates) {
  const currentMonth = new Date().getMonth() + 1;
  const existingNames = plants.filter(p => p.zone === zone && p.status !== "Harvesting" && p.status !== "Dormant").map(p => p.name.toLowerCase());
  const daysToFrost = frostDates.firstFall ? daysUntil(frostDates.firstFall) : 999;
  const candidates = CALENDAR_DATA.filter(c => {
    const plantable = [...c.indoors, ...c.transplant, ...c.direct];
    const soon = [currentMonth, currentMonth + 1, currentMonth + 2];
    const inSeason = plantable.some(m => soon.includes(m));
    const notAlreadyGrowing = !existingNames.includes(c.name.toLowerCase());
    const frostSafe = !c.dtm || daysToFrost > (c.dtm * 0.7);
    return inSeason && notAlreadyGrowing && frostSafe;
  });
  const existingCompanions = plants.filter(p => p.zone === zone).flatMap(p => p.companions?.good || []).map(c => c.toLowerCase());
  return candidates.slice(0, 5).map(c => {
    const isCompanion = existingCompanions.some(ec => ec.includes(c.name.toLowerCase()) || c.name.toLowerCase().includes(ec));
    const action = c.indoors.includes(currentMonth) ? "Start indoors" : c.transplant.includes(currentMonth) ? "Transplant now" : "Direct sow";
    return { ...c, isCompanion, action };
  });
}
...
function HarvestTab({ plants, frostDates, onUpdate }) {
  ...
  const withHarvest = plants
    .filter(p => p.status !== "Harvesting" && p.status !== "Dormant")
    .map(p => {
      const hd = calcHarvestDate(p.dateStarted, p.dtm);
      return { ...p, harvestDate: hd, daysLeft: hd ? daysUntil(hd) : null };
    })
    .filter(p => p.daysLeft !== null)
    .sort((a, b) => a.daysLeft - b.daysLeft);

  const ready = withHarvest.filter(p => p.daysLeft <= 0);
  const soon = withHarvest.filter(p => p.daysLeft > 0 && p.daysLeft <= windowDays);
  const upcoming = withHarvest.filter(p => p.daysLeft > windowDays);

  function markHarvested(plant) {
    onUpdate({ ...plant, status: "Harvesting", harvestedAt: new Date().toISOString().split("T")[0] });
  }

  function HarvestCard({ p, showAction = true }) {
    const statusObj = STATUSES.find(s => s.label === p.status) || STATUSES[0];
    const isReady = p.daysLeft <= 0;
    const isUrgent = p.daysLeft > 0 && p.daysLeft <= 7;
    return (
      <div style={{ background: "#fff", border: `1px solid ${isReady ? "#b8ddc8" : isUrgent ? "#fdd" : "#e8e8e8"}`, borderRadius: 12, padding: "12px 14px", marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3, flexWrap: "wrap" }}>
            <span style={{ fontSize: 16 }}>{statusObj.icon}</span>
            <span style={{ fontWeight: 700, fontSize: 15 }}>{p.name}</span>
            {p.variety && <span style={{ color: "#888", fontSize: 13 }}>{p.variety}</span>}
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <span style={{ fontSize: 12, color: "#999" }}>{p.zone}</span>
            {isReady
              ? <span style={{ fontSize: 12, fontWeight: 700, color: "#2d6a3f", background: "#eaf5ee", padding: "2px 8px", borderRadius: 10 }}>🎉 Ready now!</span>
              : <span style={{ fontSize: 12, color: isUrgent ? "#c0392b" : "#5a8a5a", fontWeight: isUrgent ? 700 : 400 }}>
                  🗓 {p.daysLeft} day{p.daysLeft !== 1 ? "s" : ""} — {formatDate(p.harvestDate)}
                </span>
            }
            {frostDates.firstFall && p.harvestDate && new Date(p.harvestDate) > new Date(frostDates.firstFall) && (
              <span style={{ fontSize: 11, color: "#3a5aaa", background: "#e8f0ff", padding: "2px 7px", borderRadius: 10 }}>❄️ Frost risk</span>
            )}
          </div>
        </div>
        {showAction && (
          <button onClick={() => markHarvested(p)} style={{ background: "#2d6a3f", color: "#fff", border: "none", borderRadius: 8, padding: "7px 12px", cursor: "pointer", fontSize: 12, fontWeight: 600, whiteSpace: "nowrap", flexShrink: 0 }}>
            ✓ Harvested
          </button>
        )}
      </div>
  ...
  function Section({ title, icon, items, accent, showAction = true, empty }) {
    return (
      <div style={{ marginBottom: 24 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
          <span style={{ fontSize: 18 }}>{icon}</span>
          <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700 }}>{title}</h3>
          <span style={{ background: accent, fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 20, color: "#fff" }}>{items.length}</span>
        </div>
        {items.length === 0
          ? <div style={{ color: "#bbb", fontSize: 13, padding: "8px 0" }}>{empty}</div>
          : items.map(p => <HarvestCard key={p.id} p={p} showAction={showAction} />)
        }
      </div>
    );
  }
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
        <div>
          <h2 style={{ margin: "0 0 4px", fontSize: 22, fontWeight: 800 }}>Harvest & What's Next</h2>
          <p style={{ color: "#888", margin: 0, fontSize: 14 }}>Track what's ready and what to plant in freed-up spaces.</p>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <span style={{ fontSize: 13, color: "#888" }}>Window:</span>
          {[7, 14, 30].map(d => (
            <button key={d} onClick={() => setWindowDays(d)} style={{ padding: "5px 11px", borderRadius: 8, border: "1px solid #ddd", background: windowDays === d ? "#2d6a3f" : "#fff", color: windowDays === d ? "#fff" : "#555", cursor: "pointer", fontSize: 13, fontWeight: windowDays === d ? 700 : 400 }}>{d}d</button>
          ))}
        </div>
      </div>

      <Section title="Ready to harvest" icon="🧺" items={ready} accent="#2d6a3f" empty="Nothing ready yet — check back soon." />
      <Section title={`Ripening in the next ${windowDays} days`} icon="⏳" items={soon} accent="#e67e22" empty={`Nothing ripening in the next ${windowDays} days.`} />
      <Section title="Later this season" icon="📆" items={upcoming} accent="#999" showAction={false} empty="No upcoming harvests. Add days-to-maturity to your plants to see them here." />

      <div style={{ borderTop: "2px solid #f0f0f0", paddingTop: 22, marginTop: 4 }}>
        <h3 style={{ margin: "0 0 4px", fontSize: 17, fontWeight: 700 }}>🌱 What to plant next</h3>
        <p style={{ color: "#888", fontSize: 13, marginBottom: 16 }}>In-season suggestions by zone, filtered by your frost dates and what you're already growing.</p>
        {ZONES.map(zone => {
          const suggestions = getNextPlantSuggestions(zone, plants, frostDates);
          const openingUp = plants.filter(p => p.zone === zone && (p.status === "Harvesting" || p.status === "Dormant")).length;
          if (suggestions.length === 0) return null;
          return (
            <div key={zone} style={{ marginBottom: 20 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: ZONE_COLORS[zone], display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>{ZONE_ICONS[zone]}</div>
                <span style={{ fontWeight: 600, fontSize: 14 }}>{zone}</span>
                {openingUp > 0 && <span style={{ fontSize: 11, color: "#2d6a3f", background: "#eaf5ee", padding: "2px 8px", borderRadius: 10 }}>🔄 {openingUp} space{openingUp > 1 ? "s" : ""} opening up</span>}
              </div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {suggestions.map(s => (
                  <div key={s.name} style={{ background: s.isCompanion ? "#f0f9f0" : "#fafaf8", border: `1px solid ${s.isCompanion ? "#b8ddc8" : "#e8e8e8"}`, borderRadius: 10, padding: "10px 12px", minWidth: 120, flex: "0 0 auto" }}>
                    <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 2 }}>{s.name}</div>
                    <div style={{ fontSize: 11, color: "#2d6a3f", background: "#eaf5ee", display: "inline-block", padding: "1px 7px", borderRadius: 8, marginBottom: 4 }}>{s.action}</div>
                    {s.isCompanion && <div style={{ fontSize: 11, color: "#2d6a3f", fontWeight: 600 }}>✓ companion match</div>}
                    {s.dtm && <div style={{ fontSize: 11, color: "#aaa", marginTop: 2 }}>{s.dtm}d to harvest</div>}
                  </div>
                ))}
              </div>
            </div>
